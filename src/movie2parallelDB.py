...
MERGED_MATCH_CORRELATION_THRESHOLD = 75.0
SURE_MATCH_CORRELATION_THRESHOLD = 60.0
	...
	while spa_index < len(proscript_spa.segment_list) and eng_index < len(proscript_eng.segment_list):
		...
		if correlation >= SURE_MATCH_CORRELATION_THRESHOLD:
			...
			for plus_check in range(1,MAX_MERGE_NO):
				if eng_index + plus_check < len(proscript_eng.segment_list) and proscript_eng.segment_list[eng_index].speaker_id == proscript_eng.segment_list[eng_index + plus_check].speaker_id:
					match_candidates_eng.append(eng_index + plus_check)
				else:
					break
			for plus_check in range(1,MAX_MERGE_NO):
				if spa_index + plus_check < len(proscript_spa.segment_list) and proscript_spa.segment_list[spa_index].speaker_id == proscript_spa.segment_list[spa_index + plus_check].speaker_id:
			...
			for index, candidate in enumerate(match_candidates):
				...
				#print("%i-%s - %s:%i"%(index + 1, [proscript_spa.segment_list[spa_index].id for spa_index in candidate[0]], [proscript_eng.segment_list[eng_index].id for eng_index in candidate[1]], merged_correlation))
			...
			#print("Best 2 match %i of %i"%(best_2_match_index + 1, len(match_candidates)))
			#print("Best 3 match %i of %i"%(best_3_match_index + 1, len(match_candidates)))
			...
			if not best_2_match_index == -1 and candidate_scores[best_2_match_index] >= candidate_scores[0] and candidate_scores[best_2_match_index] >= MERGED_MATCH_CORRELATION_THRESHOLD:
				#print("Merged match (%f) between spa:%s and eng:%s"%(candidate_scores[best_2_match_index], [proscript_spa.segment_list[spa_index].id for spa_index in best_2_match[0]], [proscript_eng.segment_list[eng_index].id for eng_index in best_2_match[1]]))
				...
				#print("Merged match (%f) between spa:%s and eng:%s"%(candidate_scores[best_3_match_index], [proscript_spa.segment_list[spa_index].id for spa_index in best_3_match[0]], [proscript_eng.segment_list[eng_index].id for eng_index in best_3_match[1]]))
				...
				#print("OK match (%f) between spa:%s and eng:%s"%(candidate_scores[0], [proscript_spa.segment_list[spa_index].id for spa_index in match_candidates[0][0]], [proscript_eng.segment_list[eng_index].id for eng_index in match_candidates[0][1]]))
